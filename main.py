...
writer = SummaryWriter(comment='peptide_embedding_training')
def main():
  # Init logger
  ...
  for epoch in range(args.start_epoch, args.epochs):
    ...
    # train for one epoch
    train_auc, train_los = train(train_loader, net, criterion, optimizer, epoch, log,writer)
...
def train(train_loader, model, criterion, optimizer, epoch, log,writer):
  ...
  for i, (input, target) in enumerate(train_loader):
    ...
    if i % args.print_freq == 0:
      ...
      #a1=features.data.shape[0]
      #a2=features.data.shape[1]
      #a3=features.data.shape[2]
      #features=features.data.view(a1,a2*a3)
      #aa=torch.ones(len(input_var), 1,1)
      #aa=aa.cuda()
      #input_var.data = torch.cat((input_var.data, aa), 0)
      writer.add_embedding(
        features,
        #mat=input_var.data,
        metadata=target_binary.flatten().data,
        #label_img=input_var.data,
      ...
      #for name, param in model.named_parameters():
            ...
            # Tot_W=Tot_W+param.data.abs_().sum()
            # print(name, param.data.abs_().sum())
  info = { 'train_loss': loss.item(),'train_auc': auc.item() }
  for tag, value in info.items():
        logger.scalar_summary(tag, value, epoch)
  
  for tag, value in model.named_parameters():
        #print(tag, value.data)
        tag = tag.replace('.', '/')
