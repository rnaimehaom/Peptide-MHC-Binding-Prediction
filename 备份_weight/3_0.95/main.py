...
def train(train_loader, model, criterion, optimizer, epoch, log):
	...
	for i, (input, target) in enumerate(train_loader):
		...
		y_ture=target_binary.flatten()
		if len(np.unique(y_ture)) == 1:
			auc=torch.FloatTensor([0.5])
		else:
			auc = roc_auc_score(y_ture, output.flatten())
...
def validate(val_loader, model, criterion, log,epoch):
	...
	for i, (input, target) in enumerate(val_loader):
		...
		target_binary=np.where(target>=0.42562, 1,0)
		output=output.cpu().data.numpy()
		auc_test = roc_auc_score(target_binary.flatten(), output.flatten())
		# measure accuracy and record loss
		#prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
		losses.update(loss.item(), input.size(0))
	...
	print_log('  **Test** loss@1 {loss:.3f} auc@1 {auc:.3f}'.format(loss=loss,auc=auc_test), log)
	#log_value('val_loss', loss, epoch)
	#log_value('val_auc', auc, epoch)
	info = {'valid_loss': loss.item(),'valid_auc': auc_test.item() }
	for tag, value in info.items():
				logger.scalar_summary(tag, value, epoch)
	return  auc_test, loss
