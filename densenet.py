...
class SingleLayer(nn.Module):
  def __init__(self, nChannels, growthRate):
    ...
    self.fc_w15 = nn.Parameter(torch.randn(15,1).t())
    self.fc_w34 = nn.Parameter(torch.randn(34,1).t())
    self.fc_w5 = nn.Parameter(torch.randn(5,1).t())
    self.fc_w11= nn.Parameter(torch.randn(11,1).t())
    self.fc_w17 = nn.Parameter(torch.randn(17,1).t())
  def forward(self, x):
    #print('x', x.shape)
    if x.shape[1]==InChannels:
        ...
        x2=x[:,InChannels:,:,:]
        #print('InChannels',InChannels)##为什么会无限多次进入呢？？！！！！ ？？？？？？
        #xxx=x1
        ...
        if long==15:
            ...
            weight=torch.sigmoid(F.linear(x_outt,self.fc_w15)+F.linear(x11,self.fc_w15))
            #x1=weight.t()*x1
            x1=x1.detach()
            #print(x1.shape)
            #print(weight.shape)
            ...
            w1=weight.clone().expand(64,8)
            w1=w1.repeat(1,1,15)
            w1=w1.view(64,8,15,1)
            x1=w1*x1.clone()
            xxx=x1
            for i in range(n-2,-1,-1):
                ...
                weight=torch.sigmoid(F.linear(xx_temp,self.fc_w15)+F.linear(x_outt,self.fc_w15))
                xx=xx.detach()
                w1=weight.clone().expand(64,4)
                w1=w1.repeat(1,1,15)
                w1=w1.view(64,4,15,1)
                xx=w1*xx.clone()
            ...
            x_outt=torch.mean(x_out,dim=1)
            x_outt = torch.squeeze(input=x_outt)
            weight=torch.sigmoid(F.linear(x_outt,self.fc_w34)+F.linear(x11,self.fc_w34))
            x1=x1.detach()
            w1=weight.clone().expand(64,8)
            w1=w1.repeat(1,1,34)
            w1=w1.view(64,8,34,1)
            x1=w1*x1.clone()
            xxx=x1
            for i in range(n-2,-1,-1):
                #xx=x2[:,(i*growth):(i+1)*growth,:,:]
                ...
                #print(xx_temp.shape)
                xx_temp = torch.squeeze(input=xx_temp)
                weight=torch.sigmoid(F.linear(xx_temp,self.fc_w34)+F.linear(x_outt,self.fc_w34))
                xx=xx.detach()
                w1=weight.clone().expand(64,4)
                w1=w1.repeat(1,1,34)
                w1=w1.view(64,4,34,1)
                xx=w1*xx.clone()
                xxx=torch.cat((xxx,xx),1)
                i=i+1
        elif long==5:
            ...
            x_outt=torch.mean(x_out,dim=1)
            x_outt = torch.squeeze(input=x_outt)
            weight=torch.sigmoid(F.linear(x_outt,self.fc_w5)+F.linear(x11,self.fc_w5))
            x1=x1.detach()
            w1=weight.clone().expand(64,8)
            w1=w1.repeat(1,1,5)
            w1=w1.view(64,8,5,1)
            ...
            for i in range(n-2,-1,-1):
                ...
                #print(xx_temp.shape)
                xx_temp = torch.squeeze(input=xx_temp)
                weight=torch.sigmoid(F.linear(xx_temp,self.fc_w5)+F.linear(x_outt,self.fc_w5))
                xx=xx.detach()
                w1=weight.clone().expand(64,4)
                w1=w1.repeat(1,1,5)
                w1=w1.view(64,4,5,1)
            ...
            x_outt=torch.mean(x_out,dim=1)
            x_outt = torch.squeeze(input=x_outt)
            weight=torch.sigmoid(F.linear(x_outt,self.fc_w11)+F.linear(x11,self.fc_w11))
            x1=x1.detach()
            w1=weight.clone().expand(64,8)
            w1=w1.repeat(1,1,11)
            w1=w1.view(64,8,11,1)
            x1=w1*x1.clone()
            xxx=x1
            for i in range(n-2,-1,-1):
                #xx=x2[:,(i*growth):(i+1)*growth,:,:]
                ...
                #print(xx_temp.shape)
                xx_temp = torch.squeeze(input=xx_temp)
                weight=torch.sigmoid(F.linear(xx_temp,self.fc_w11)+F.linear(x_outt,self.fc_w11))
                xx=xx.detach()
                w1=weight.clone().expand(64,4)
                w1=w1.repeat(1,1,11)
                w1=w1.view(64,4,11,1)
                xx=w1*xx.clone()
                xxx=torch.cat((xxx,xx),1)
                i=i+1
            ...
            x_outt=torch.mean(x_out,dim=1)
            x_outt = torch.squeeze(input=x_outt)
            weight=torch.sigmoid(F.linear(x_outt,self.fc_w17)+F.linear(x11,self.fc_w17))
            x1=x1.detach()
            w1=weight.clone().expand(64,8)
            w1=w1.repeat(1,1,17)
            w1=w1.view(64,8,17,1)
            x1=w1*x1.clone()
            xxx=x1
            for i in range(n-2,-1,-1):
                #xx=x2[:,(i*growth):(i+1)*growth,:,:]
                ...
                #print(xx_temp.shape)
                xx_temp = torch.squeeze(input=xx_temp)
                weight=torch.sigmoid(F.linear(xx_temp,self.fc_w17)+F.linear(x_outt,self.fc_w17))
                xx=xx.detach()
                w1=weight.clone().expand(64,4)
                w1=w1.repeat(1,1,17)
                w1=w1.view(64,4,17,1)
                xx=w1*xx.clone()
                xxx=torch.cat((xxx,xx),1)
                i=i+1
...
class DenseNet(nn.Module):
  ...
  def forward(self, x):
    ...
    out_all_f=torch.squeeze(out_all)
    #print('out_all',out_all.shape)
    #out_all = torch.squeeze(F.avg_pool2d(F.relu(self.bn1(out_all),inplace=True), 2))
    out_all = self.fc(out_all_f)#64,56
    out=torch.sigmoid(out_all)
    return out,out_all_f

def densenet100_12(num_classes=1):
  model = DenseNet(4, 16, 1, num_classes, False)
  return model
'''
dummy_input = (torch.zeros(1, 21,49),)
with SummaryWriter(comment='densenet100_12') as w:
    w.add_graph(densenet100_12(), dummy_input, True)
'''